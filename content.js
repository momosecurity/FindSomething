...
(function(){
    browser.storage.local.get(["expire_index"], function(expire_index){
        ...
        Object.keys(expire_index).forEach(key => {
            ...
            if (expire_index[key]<sevenDaysAgostr) {
                ...
                browser.storage.local.remove(["findsomething_result_"+key], function() {});
            }
        });
        if(reset){
            // console.log("重新设置expire_index：")
            // console.log(expire_index)
            browser.storage.local.set({["expire_index"]: expire_index}, function(){} )
        }
    })
